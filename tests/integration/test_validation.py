...
class TestPHPValidation:
    ...
    def test_contains_edges(self, db):
        edges = _get_edges(db)
        contains = {(e["from_name"], e["to_name"]) for e in edges if e["type"] == "contains"}
        assert any("Tracker" in f and t == "track" for f, t in contains)
        assert any("Tracker" in f and t == "flush" for f, t in contains)
        assert any("ClickTracker" in f and t == "trackClick" for f, t in contains)
    ...
    def test_calls_edges(self, db):
        ...
        assert any(t == "track" for _, t in calls)
